        ...
        Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
            FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                StatusChip(
                    ...
                    if (synthesis == null) stringResource(R.string.unavailable) else synthesis.snapshot.freshness.name.lowercase(),
                    synthesis != null,
                )
            }
...
fun ReportsScreen(model: AppViewModel) {
    ...
    var selected by remember(completed) { mutableStateOf(completed.firstOrNull()?.id) }
    ...
    Page(stringResource(R.string.reports), stringResource(R.string.report_local_only)) {
        Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
            if (completed.isEmpty()) Text(stringResource(R.string.select_session_report))
            completed.forEach { session ->
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    Text(session.label, Modifier.weight(1f))
                    RadioButton(selected = selected == session.id, onClick = { selected = session.id })
                }
            }
            Button(
                enabled = selected != null,
                onClick = { selected?.let { pendingJson = it; jsonLauncher.launch("fluxlab-$it.json") } },
                modifier = Modifier.fillMaxWidth(),
            ) { Text(stringResource(R.string.export_json)) }
            OutlinedButton(
                enabled = selected != null,
                onClick = { selected?.let { pendingCsv = it; csvLauncher.launch("fluxlab-$it.csv") } },
                modifier = Modifier.fillMaxWidth(),
            ...
            comparison?.let { value ->
                ...
                Button(
                    onClick = { comparisonJsonLauncher.launch("fluxlab-comparison.json") },
                    modifier = Modifier.fillMaxWidth(),
                ) { Text(stringResource(R.string.export_comparison_json)) }
                OutlinedButton(
                    onClick = { comparisonCsvLauncher.launch("fluxlab-comparison.csv") },
                    modifier = Modifier.fillMaxWidth(),
                ) { Text(stringResource(R.string.export_comparison_csv)) }
